...
public class HelloController {
    ...
    @RequestMapping("/view2/{courceId}")
    public String viewCourse2(@PathVariable("courceId") Integer courseId, Map<String, Object> map){
        User user = userService.getMapper().selectByPrimaryKey(courseId);
        map.put("user",user);
        return "/course_overview";
    }
    ...
    @RequestMapping(value = "/admiin", method = RequestMethod.GET, params = "add")
    ...
    public void findName(@PathVariable String name){
        System.out.println("name: " + name);    //  /listWin    name: Win
    }

    @RequestMapping(value="/list2{name}")
    public void findNameAlternative(@PathVariable("name") String someName){
        System.out.println("name: " + someName);    //  /list2Win    name: Win
    }
}
